...
class TestComputeSpinVectorPoint:
    ...
    def test_point_mode_returns_9d_vector(self):
        ...
        spin, phi_centers, phi_starts, phi_ends = compute_spin_vector(timestamp)
        ...
        assert spin[2] == 0.0, "Quarter scale z should be 0 for points"
        assert spin[5] == 0.0, "Decade scale z should be 0 for points"
        assert spin[8] == 0.0, "Century scale z should be 0 for points"
    ...
    def test_point_mode_unit_circle_constraint(self):
        ...
        # Check each scale
        assert is_unit_circle_point(spin[0], spin[1]), "Quarter scale not on unit circle"
        assert is_unit_circle_point(spin[3], spin[4]), "Decade scale not on unit circle"
        assert is_unit_circle_point(spin[6], spin[7]), "Century scale not on unit circle"
    ...
    def test_point_mode_phase_progression(self):
        ...
        assert abs(quarter_diff - math.pi/2) < 0.01, "Quarter scale should show π/2 for 3 months"
    
    def test_point_mode_same_timestamp_identical_encoding(self):
        """Same timestamp should produce identical spin vectors."""
        timestamp = datetime(2023, 5, 15, 12, 30, 45, tzinfo=timezone.utc).timestamp()
...
class TestComputeSpinVectorArc:
    ...
    def test_arc_mode_returns_9d_vector_with_nonzero_z(self):
        ...
        spin, phi_centers, phi_starts, phi_ends = compute_spin_vector(
    ...
    def test_arc_mode_quarterly_arc_length(self):
        ...
        assert abs(quarter_arc - expected) < 0.1, (
    ...
    def test_arc_mode_full_year_arc_length(self):
        ...
        assert abs(quarter_arc - math.tau) < 0.2, (
    ...
    def test_arc_mode_phi_starts_ends_defined(self):
        ...
        for scale in ['quarter', 'decade', 'century']:
            ...
            assert is_valid_phase(phi_starts[scale])
            assert is_valid_phase(phi_ends[scale])
...
class TestAngularDifference:
    ...
    def test_same_angle_zero_difference(self):
        ...
        phi = math.pi / 4
        assert angular_difference(phi, phi) == 0.0
    ...
    def test_commutative_property(self):
        ...
        phi1 = 1.5
        phi2 = 4.2
        assert abs(angular_difference(phi1, phi2) - angular_difference(phi2, phi1)) < 1e-10
...
class TestArcOverlap:
    ...
    def test_wrapping_arcs(self):
        ...
        overlap = arc_overlap(5*math.pi/4, math.pi/4 + math.tau, 0.0, math.pi/2)
...
class TestJaccardSimilarityArcs:
    ...
    def test_non_overlapping_arcs_similarity_zero(self):
        ...
        similarity = jaccard_similarity_arcs(0.0, math.pi/4, math.pi/2, 3*math.pi/4)
